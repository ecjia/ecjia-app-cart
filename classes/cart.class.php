...
// 			}
		...
		foreach ($arr AS $key => $val) {
			...
			if (intval(ecjia::config('use_storage')) > 0 && $cart['extension_code'] != 'package_buy') {
				...
				if (!empty($cart['product_id'])) {
					$product_number = RC_DB::table('products')->where('goods_id', $cart['goods_id'])->where('product_id', $cart['product_id'])->value('product_number');
					if ($product_number < $val) {
						return new ecjia_error('low_stocks', __('库存不足', 'cart'));
					}
				}
	...
	public static function cart_amount($include_gift = true, $type = null, $cart_id = array()) {
		...
		$data = $db->value(RC_DB::raw('sum(goods_price * goods_number)'));
...
// 		$packages_row['number'] = intval($row['number']) + $package_row['number'];
	...
	public static function formated_weight($weight) {
		...
		if ($weight > 0) {
			if ($weight < 1) {
				/* 小于1千克，用克表示 */
				return intval($weight * 1000) . __('克', 'cart');
			} else {
				/* 大于1千克，用千克表示 */
				return $weight . __('千克', 'cart');
			}
		} else {
			return 0;
		}
